...
export default function useLookIntoTheAshes({ game, myPlayer, discardDeck }: Params) {
  const [open, setOpen] = useState(false);
  const [cards, setCards] = useState<Card[]>([]);

  // solo si me toca
  const enable = !!game && !!myPlayer && game.status === "waiting_for_choose_discarded" && game.player_in_action === myPlayer.id;
  ...
  const confirm = async (chosenDiscardedId: number) => {
    ...
    await CardService.playCardWithTargets(eventCard.id , myPlayer.token, {
      target_players: [],
      target_secrets: [],
      target_cards: [chosenDiscardedId],
      target_sets: [],
    });
    setOpen(false);
    setCards([]);
